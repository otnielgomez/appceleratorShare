...
function BirdHouse(params) {
	...
	function api2(url, method, params, callback, auth, setUrlParams, setHeader) {
		...
		if (!authorized && ( typeof (auth) == 'undefined' || auth === true)) {
			...
			authorize(function(retval) {
				...
				if (!retval) {
					...
					api2(url, method, params, callback, auth);
				}
			});
		}
		// user is authorized so execute API
			...
			Titanium.API.info('finalUrl  :' + finalUrl);
			Titanium.API.info('method  :' + method);
			...
			if ( typeof (setHeader) == 'undefined' || setHeader == true) {
				...
				for (var i = 0; i < message.parameters.length; i++) {
					...
					Titanium.API.info('header  :' + header);
					header = header + message.parameters[i][0] + '="' + escape(message.parameters[i][1]) + '"';
					Titanium.API.info('header 2  :' + header);
				}
	...
	function send_tweet(params, callback) {
		...
		//api2('http://api.twitter.com/1/statuses/update.json', 'POST', params, function(resp) {
		api2('https://api.twitter.com/1.1/statuses/update.json', 'POST', params, function(resp) {
			if (resp != false) {
